...
<div class="px-6 py-8">
    ...
    <form action="{{ route('admin.coupons.update', $coupon) }}" method="POST" class="space-y-6">
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
            ...
            <!-- Usage Statistics -->
            <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                <h2 class="mb-4 text-lg font-semibold text-gray-900">Usage Statistics</h2>
                
                <div class="grid grid-cols-2 gap-4">
                    <div class="rounded-lg bg-blue-50 p-4">
                        <p class="text-sm text-gray-600">Times Used</p>
                        <p class="text-2xl font-bold text-blue-600">{{ $coupon->usage_count ?? 0 }}</p>
                    </div>
                    <div class="rounded-lg bg-green-50 p-4">
                        <p class="text-sm text-gray-600">Total Saved</p>
                        <p class="text-2xl font-bold text-green-600">${{ number_format($coupon->total_saved ?? 0, 2) }}</p>
                    </div>
                    <div class="rounded-lg bg-purple-50 p-4">
                        <p class="text-sm text-gray-600">Unique Users</p>
                        <p class="text-2xl font-bold text-purple-600">{{ $coupon->unique_users ?? 0 }}</p>
                    </div>
            ...
            <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="number" name="usage_limit" id="usage_limit" value="{{ old('usage_limit', $coupon->usage_limit) }}" min="0"
                            class="mt-1 block w-full rounded-lg border border-gray-300 px-4 py-2 text-gray-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
                    ...
                    <div>
                        ...
                        <input type="number" name="usage_limit_per_user" id="usage_limit_per_user" value="{{ old('usage_limit_per_user', $coupon->usage_limit_per_user) }}" min="0"
                            class="mt-1 block w-full rounded-lg border border-gray-300 px-4 py-2 text-gray-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
                            placeholder="Leave empty for unlimited">
                    ...
                    <div>
                        ...
                        <div class="relative mt-1">
                            ...
                            <input type="number" name="min_purchase_amount" id="min_purchase_amount" value="{{ old('min_purchase_amount', $coupon->min_purchase_amount) }}" step="0.01" min="0"
                    ...
                    <div>
                        ...
                        <div class="relative mt-1">
                            <span class="absolute left-4 top-1/2 -translate-y-1/2 text-sm text-gray-500">$</span>
                            <input type="number" name="max_discount_amount" id="max_discount_amount" value="{{ old('max_discount_amount', $coupon->max_discount_amount) }}" step="0.01" min="0"
                                class="block w-full rounded-lg border border-gray-300 px-4 py-2 pl-8 text-gray-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
                                placeholder="0.00">
                        </div>
            ...
            <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="datetime-local" name="starts_at" id="starts_at" value="{{ old('starts_at', $coupon->starts_at ? \Carbon\Carbon::parse($coupon->starts_at)->format('Y-m-d\TH:i') : '') }}"
                            class="mt-1 block w-full rounded-lg border border-gray-300 px-4 py-2 text-gray-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500">
                        @error('starts_at')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="datetime-local" name="expires_at" id="expires_at" value="{{ old('expires_at', $coupon->expires_at ? \Carbon\Carbon::parse($coupon->expires_at)->format('Y-m-d\TH:i') : '') }}"
                            class="mt-1 block w-full rounded-lg border border-gray-300 px-4 py-2 text-gray-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500">
                        @error('expires_at')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <select name="applicable_to" id="applicable_to"
                            ...
                            <option value="specific_products" {{ old('applicable_to', $coupon->applicable_to) == 'specific_products' ? 'selected' : '' }}>Specific Products</option>
                            <option value="specific_categories" {{ old('applicable_to', $coupon->applicable_to) == 'specific_categories' ? 'selected' : '' }}>Specific Categories</option>
                        </select>
                        @error('applicable_to')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="flex items-center gap-3">
                        <input type="checkbox" name="exclude_sale_items" id="exclude_sale_items" value="1" {{ old('exclude_sale_items', $coupon->exclude_sale_items) ? 'checked' : '' }}
                            class="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500">
